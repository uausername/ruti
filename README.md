...
| `ruti mode flow {on\|off}`, `ruti flow handoff` | Hand the task to a fresh session at 50% context: the handoff is written by the manager, the new window is opened by the `Stop` hook, and the new session inherits the handoff, the modes and the permission mode. | `/compact` keeps whatever a summariser chose to keep; flow keeps exactly what the manager writes down for its successor — and does it at 50%, while there is still room to write it well. |
